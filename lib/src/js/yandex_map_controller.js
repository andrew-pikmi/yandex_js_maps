    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!window.ymaps3Clusterer) {
          window.ymaps3Clusterer = await window.ymaps3.import('@yandex/ymaps3-clusterer@0.0.11');
        }
        ...
        const clusterer = new YMapClusterer({
          method: (function () {
            const gridMethod = clusterByGrid({ gridSize: clusterData.options.gridSize || 64 });
            const maxZoom = clusterData.options.maxZoom ?? null;
            if (maxZoom === null) return gridMethod;
            return {
              render({ map, features }) {
                if (Math.round(map.zoom) > maxZoom) {
                  // Above maxZoom — show individual markers, no clustering
                  return features.map((f) => ({ ...f, clusterId: f.id }));
                }
                return gridMethod.render({ map, features });
              },
            };
          })(),
          features,
          ...
          cluster(coordinates, clusterFeatures) {
            ...
            if (typeof window[builderKey] === 'function') {
              ...
              const dataUrl = window[builderKey](ids);
              if (dataUrl) options = { ...options, iconDataUrl: dataUrl };
            }
