...
window.yandexMapHelpers.createPlacemarkElement = function (properties = {}, options = {}) {
  ...
  // Default circle icon — all values from Dart options, no hardcoded fallbacks beyond safety checks
  const el = document.createElement('div');
  el.style.width = `${size}px`;
  el.style.height = `${size}px`;
  el.style.borderRadius = '50%';
  el.style.border = `${options.borderWidth || 0}px solid ${options.borderColor || 'transparent'}`;
  el.style.boxSizing = 'border-box';
  if (options.hasShadow) {
    el.style.boxShadow = '0 2px 8px rgba(0, 0, 0, 0.35)';
  }
  el.style.background = options.iconColor || '#ff4d4f';
...
window.yandexMapHelpers.createClusterMarker = function (coordinates, count, options, clusterId) {
  ...
  if (options.iconDataUrl) {
    ...
    wrap.style.cssText = `position:relative;width:${size}px;height:${size}px;transform:translate(-50%,-50%);cursor:pointer;`;
    const img = document.createElement('img');
    img.src = options.iconDataUrl;
    img.width = size;
    img.height = size;
    ...
    if (options.showClusterCount !== false) {
    ...
    el.style.background = options.clusterColor || '#1e98ff';
    el.style.color = '#ffffff';
    el.style.display = 'flex';
    el.style.alignItems = 'center';
    el.style.justifyContent = 'center';
